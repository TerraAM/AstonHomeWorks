...
public class ModalWindow {
		...
		private void action(By byXmlElement) {
			try {
	            ...
	            if (cookiesButton.isDisplayed()) {
	                cookiesButton.click();
	                Allure.step("Баннер cookie пройден");
	            }
	        } catch (Exception e) {
	        	Allure.step("Баннер cookie не найден");
	        }
		}
	}
	...
	public class PaymentFrame{
		...
		public boolean areLabelsHaveText(Map<String, String> expectedLabels) {
			...
			Allure.step("Пытаемся поменять рабочий фрейм...");
		    ...
		    Allure.step("Меняем фрейм на: " + rootIframeName);
		    ...
		    for (Map.Entry<String, String> entry : expectedLabels.entrySet()) {
		        ...
		        String message = String.format("(Class: %s) %s (Результат: %s) : %b", className, expectedText, actualText, match);
		        Allure.step(message);
		    }
		...
		public boolean areAmountCompare(String expectedAmount) {
		    ...
		    String message = String.format("Сумма в <span>: %s, ожидаемая: %s - %b%n"
		    							 + "Сумма в <button>: %s, ожидаемая: %s - %b%n",
		    							 spanText, formattedExpectedAmount, spanMatches,
		    							 buttonText, formattedExpectedAmount, buttonMatches);
	        Allure.step(message);
