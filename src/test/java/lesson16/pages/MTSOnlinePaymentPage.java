...
public class MTSOnlinePaymentPage {
    ...
    public String getBlockTitle() {
    	System.out.println("Полученное сообщение: " + driver.findElement(blockTitle).getText().replace("\n", " ").trim());
    ...
    public boolean areAllLogosDisplayed(String[] expectedAltTexts) {
        ...
        for (String altText : expectedAltTexts) {
            ...
            try {
                ...
                System.out.printf("Логотип с alt=%s отображается: %b%n", altText, isDisplayed);
                allMatch &= isDisplayed;
            } catch (NoSuchElementException e) {
                System.out.printf("Логотип с alt=%s не найден %n", altText);
                allMatch = false;
            }
        }
        return allMatch;
    ...
    public MTSOnlinePaymentPage clickMoreInfoButton() {
        ...
        System.out.println("Результат перехода по ссылке: " + getCurrentUrl());
        return this;
    }
    ...
    public MTSOnlinePaymentPage fillConnectionForm(String phone, String sum, String email) {
        ...
        System.out.printf("Заполнение даннными:%n"
				 + "Тел.: %s%n"
				 + "Сум.: %s%n"
    ...
    public boolean checkPlaceholders(Map<String, String> expectedPlaceholders) {
        ...
        for (Map.Entry<String, String> entry : expectedPlaceholders.entrySet()) {
            ...
            System.out.printf("(Id: %s) %s (Результат: %s) : %b%n", id, expectedPlaceholder, actualPlaceholder, match);
        }
